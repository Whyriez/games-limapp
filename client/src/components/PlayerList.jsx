...
import { Users, Play, Radio, Sparkles, ChevronDown, ChevronUp } from "lucide-react";
          ...
          <div className="md:hidden flex items-center justify-between gap-2 py-1 px-2 rounded-xl bg-[#FFFBF5] border border-[#F0DDC5]">
            <div className="flex items-center -space-x-2 overflow-hidden py-1">
              {players.slice(0, 7).map((p) => (
                <div key={p.playerId || p.socketId} className="ring-2 ring-white rounded-full">
                  <Avatar name={p.name} size="xs" isAlive={!p.isSpectator && p.isAlive} />
                </div>
              ))}
              {players.length > 7 && (
        ...
        >
          {players.map((player) => {
            const isMe = Boolean(
              ...
              (currentPlayerId && player.playerId && player.playerId === currentPlayerId) ||
              (currentNickname && player.name && player.name.trim().toLowerCase() === currentNickname.trim().toLowerCase())
            );
            const isTurn =
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                  ...
                  <div className="truncate">
                    <div className="flex items-center gap-1.5 truncate">
                      <span className={`font-extrabold text-xs sm:text-sm truncate ${!player.isAlive && !isSpectating ? "line-through text-[#A69989]" : "text-[#3A332C]"}`}>
                        {player.name}
                      </span>
                      {isMe && (
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  {!isSpectating && status === "DISCUSSION_PHASE" && player.isAlive && discussionReadySocketIds.includes(player.socketId) && (
            ...
            <div className="space-y-3">
              {canStart ? (
                <div className="flex items-center gap-1.5 text-xs font-black text-[#1C8BE0] bg-[#EFF8FF] p-2.5 rounded-xl border border-[#8CD3FF]">
                  ...
                  <span>Pemain sudah siap ({activeCount}/{minPlayersNeeded})! Klik tombol untuk memulai {gameMeta.name}:</span>
                </div>
              ) : (
                <div className="text-center text-xs font-bold text-[#8C8275] py-2.5 bg-[#FFFBF5] rounded-xl border border-[#F0DDC5]">
                  ⏳ Butuh minimal <strong>{minPlayersNeeded} pemain</strong> untuk memulai {gameMeta.name} (Sekarang: {activeCount}). Ajak temanmu bergabung!
                </div>
              )}
              ...
              >
                ...
                <span>
                  {canStart ? `Mulai Game ${gameMeta.name}` : `Mulai Game (Min. ${minPlayersNeeded} Pemain)`}
                </span>
              </button>
            </div>
          ) : (
